...
/*void Shape::setScaling(float x, float y, float z) {
	scaling_x = x;
	scaling_y
}*/
void Shape::calculateStuff(int nVerts, float *vertices) {
	/*float max_arr[3] = {FLT_MIN, FLT_MIN, FLT_MIN};
	float min_arr[3] = {FLT_MAX, FLT_MAX, FLT_MAX};*/
	//cout << *vertices;
	...
	/*
	for (int j = 1; j < nVerts/3; j+=3) {
		if (vertices[(9*i)+(3*v)+1] < min_arr[1]) {
			min_arr[1] = vertices[j];
		}
		if (vertices[j] > max_arr[1]) {
			max_arr[1] = vertices[j];
		}
	}
	for (int k = 2; k < nVerts/3; k+=3) {
		if (vertices[k] < min_arr[2]) {
			min_arr[2] = vertices[k];
		}
		if (vertices[k] > max_arr[2]) {
			max_arr[2] = vertices[k];
		}
	}*/
  ...
  cout << "maximum values: " << max_arr[0] << ", " << max_arr[1] << ", " << max_arr[2] << "\n\n";
	...
	//shape.getCameraMatrix().c.multiply(shape.translation);

	
	/*
	Matrix4 trans_old = Matrix4(shape.translation);
	shape.translation.multiply(shape.getMatrix());
	shape.setMatrix(shape.translation);
	shape.translation = Matrix4(trans_old);
	*/
	...
	//shape.getCameraMatrix().c.scale(scaling_x, scaling_y, scaling_z);
	/*
	float x_diff = min(512-max_arr[0], 512+min_arr[0]);
	float y_diff = min(512-max_arr[1], 512+min_arr[1]);
	//float z_diff = min(512-max_arr[2], 512+min_arr[2]);

	float max1 = min(x_diff, y_diff);
	//float max2 = min(max1, z_diff);

	float scaling_factor = ((float)512)/max1;
	*/

	cout << '\n' << x_diff << ", " << y_diff; 
	cout << "\nscaling factor : " << scaling_x << ", " << scaling_y <<  ", " << scaling_z <<"\n";
...
void Shape::spin(double deg)
{
  if (shape.angle > 360.0 || shape.angle < -360.0) shape.angle = 0.0;
	shape.getCameraMatrix().c.rotateCubeY(deg);
}
...
void Window::processSpecialKeys(int key, int x, int y)
{
	...
	{	
		...
		case GLUT_KEY_F8:
			...
			shape.getCameraMatrix().c.transpose();
			shape.getCameraMatrix().inverse();
			//shape.calculateStuff(shape.house_nVerts, shape.house_vertices);
			break;
		case GLUT_KEY_F9:
			// show house view2
			shape_key = 9;
